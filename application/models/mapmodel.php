...
*/      
    	...
    	if($query->num_rows() > 0) {
    		foreach($query->result_array() as $row)
    		{
    			...
    			$query2 = $this->db->query('SELECT data_id, image_id, genus, species, age, country, place, collector, lat, lng FROM ' . $row["data_table"].' WHERE '.$where_string.' and lat<> NULL limit 100');
    ...
    function loadCollector(){
        ...
        if($query->num_rows() >0) {
            ...
            {
                //we retrieve the data from each fossil from each project
                $query2=$this->db->query('SELECT distinct  collector FROM '.$row['data_table']);
    ...
    function updatelocation($data){
        ...
        if($query->num_rows() > 0) {
            ...
            {
                //we retrieve the data from each fossil from each project
                $query2 = $this->db->query('SELECT data_id, image_id, genus, species, age, country, place, collector FROM ' . $row["data_table"].' WHERE '.$where_string.' limit 10, 90');
